...
export default function AudioPlayer() {
    const showPlayer = useSelector((state) => !!state.audio.whatIsPlaying?.file ?? false)
    ...
    useEffect(() => {
        if (showPlayer) {
            document.getElementsByClassName('audio-player--audio')[0].currentTime = episode?.timestamp ??0
    ...
    const setAudioSession = async(episode, timestamp) => {
        ...
        const { data, error } = await supabase
            .from('audio_session')
            .upsert({ user_id:user.id, podcast_id:parseInt(episode.podcast_id), season:episode.season, episode:episode.episode, timestamp:timestamp,file:episode.file, title:episode.title })
    }
    useEffect(() => {
        const audioSessionSetter= setInterval(() => {
            if (showPlayer) {
                setAudioSession(episode, Math.floor(document.getElementsByClassName('audio-player--audio')[0].currentTime))
            }
        }, 5000)
